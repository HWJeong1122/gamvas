...
class plotter:
    ...
    def draw_trplot(self,
        result=None, weight=None, nmod=None, ifsingle=True, spectrum="spl", set_spectrum=True,
    ...
    def draw_cnplot(self,
        result=None, nmod=None, ifsingle=True, spectrum="spl", set_spectrum=True,
        fontsize=15, fontweight="bold", save_path=False, save_name=False, save_form="png"
    ):
        """
        draw corner plot
        ...
        for i in range(nmod):
            ...
            fig_cn, axes_cn = dyplot.cornerplot(
                ...
                dims=list(range(nidx+add,nidx+add+len(field))), labels=field, label_kwargs={"fontsize":fontsize, "fontweight":fontweight}
            )
            ...
            for i in range(nfreq):
                ...
                for j in range(nfig):
                    ...
                    for k in range(nax):
                        ...
                        plot_ax.set_rasterized(True)
                        plot_ax.errorbar(
                            clq_obs_2["time"], r2d*clq_obs_2["clphs"], r2d*clq_obs_2["sigma_clphs"],
                            marker="o", markersize=6, c="black", ls="", mfc="black", mec="dimgray",
                            capsize=6, capthick=1, zorder=1,
                            label=f"{utria[10*j+k]} ({ufreq[i]:.1f} GHz)")
                        if model:
                            plot_ax.plot(
                                clq_obs_2["time"], r2d*clq_mod_2,
                                marker="o", markersize=4, c="red", ls=":", zorder=2
                            )
                        plot_ax.xaxis.set_major_locator(MultipleLocator(2.0))
                        plot_ax.xaxis.set_minor_locator(MultipleLocator(1.0))
    ...
    def draw_fits_image(self,
        ...
        if select.lower() == "i":
            ...
            rms = uvf.fits_image_rms_i
            cntr = uvf.fits_imgcntr_i
            cmap = "gist_heat"
            vmin = cmap_snr_i*rms
            vmax = np.max(image)
        elif select.lower() == "q":
            image = uvf.fits_image_q
            rms = uvf.fits_imgrms_q
            cntr = uvf.fits_imgcntr_q
            cmap = "gist_heat"
            ...
            rms = uvf.fits_imgrms_u
            cntr = uvf.fits_imgcntr_u
            cmap = "gist_heat"
            peak_min = np.nanmin(image)
            peak_max = np.nanmax(image)
            if np.abs(peak_min) > np.abs(peak_max):
        ...
        if np.all([xlim, ylim]):
            if select.lower() != "p":
                if lim_list:
                    ...
                    beam = patches.Ellipse(
                        (0.8*(xlim[1]), 0.8*(ylim[0])),
                        uvf.fits_bmin*d2m, uvf.fits_bmaj*d2m,
                        angle=-uvf.fits_bpa, fc="grey", ec=ec, lw=1.0
                    )
                else:
                    ax_imap.set_xlim(-xlim, +xlim)
                    ax_imap.set_ylim(-ylim, +ylim)
                    beam = patches.Ellipse(
                        (0.8*(+xlim), 0.8*(-ylim)),
                        uvf.fits_bmin*d2m, uvf.fits_bmaj*d2m,
                        angle=-uvf.fits_bpa, fc="grey", ec=ec, lw=1.0
                ...
                if lim_list:
                    ...
                    ax_pmap.set_xlim(-xlim, +xlim)
                    ax_pmap.set_ylim(-ylim, +ylim)
                    ax_fmap.set_xlim(xlim[0], xlim[1])
                    ax_fmap.set_ylim(ylim[0], ylim[1])
                    beam = patches.Ellipse(
                        (0.8*(+xlim), 0.8*(-ylim)),
            ...
            xlim = np.round(np.max(ra), 1)
            ylim = np.round(np.max(dec), 1)
            beam = patches.Ellipse(
                (0.8*(+xlim), 0.8*(-ylim)),
                uvf.fits_bmin*d2m, uvf.fits_bmaj*d2m,
                angle=-uvf.fits_bpa, fc="grey", ec=ec, lw=1.0
    ...
    def generate_image(self,
        uvf, freq_ref=False, freq=False, prms=False, ifsingle=True, set_spectrum=False, spectrum="spl"
    ):
        """
