...
class CreateArray:
    def __init__(
        self,
        source=None, source_coord=None, frequency=None, array=None, tarr=None,
        date=None, tstart=None, duration=None, scanlen=None, gaptime=None,
        tint=None, ellim=0
    ):
        """
        Args:
            ...
            source_coord (astropy.coordinates.SkyCoord): Coordinates of
                the source.
            frequency (float, array): Frequency of the observation in Hz.
    ...
    def carr(self, returned=True):
        if isinstance(self.frequency, (int, float)):
            self.frequency = [self.frequency]

        for nf, frequency in enumerate(self.frequency):
            obsw = (ac.c / (frequency * au.Hz)).to(au.m)
            mjd0 = atime(self.date, format="iso").mjd
            scantime = np.arange(
                self.tstart,
            ...
            for nscan, scantime_ in enumerate(scantime):
                time = np.append(
                    time,
                    scantime_ + np.arange(
                        0, nvis_scan * self.tint, self.tint
                    ) / 3600
                )

            # Time information in UTC
            times = atime(mjd0 + time / 24, format="mjd").iso

            # Load tarr information
            array = self.array
            tarr = self.tarr

            if tarr is None and array is None:
                raise ValueError("Array information is not given.")

            if tarr is None:
                tarr, array = get_vlbi(array)
            ...
            self.ant_dict_name2num = dict(zip(
                [str(name) for name in self.tarr["name"]],
                [number for number in range(1, self.tarr.shape[0] + 1)]
            ))

            self.ant_dict_num2name = {
                val: key
                for key, val in self.ant_dict_name2num.items()
            }
            ...
            baselines = [
                256 * int(x) + int(y)
                for x, y in itertools.combinations(tarr["number"], 2)
            ]
            ...
            for ntime, time in enumerate(times):
                for baseline in baselines:
                    ...
                    obs_ant1.append(ant1)
                    obs_ant2.append(ant2)

            obs_ant1_name = np.array(list(
                map(self.ant_dict_num2name.get, obs_ant1)
            ))
            obs_ant2_name = np.array(list(
                map(self.ant_dict_num2name.get, obs_ant2)
            ))

            dict_lat = dict(zip(tarr["number"], tarr["lat"]))
            dict_lon = dict(zip(tarr["number"], tarr["lon"]))
            dict_hgt = dict(zip(tarr["number"], tarr["height"]))

            arr_lat1 = np.array(list(map(dict_lat.get, obs_ant1)))
            arr_lon1 = np.array(list(map(dict_lon.get, obs_ant1)))
            arr_hgt1 = np.array(list(map(dict_hgt.get, obs_ant1)))
            arr_lat2 = np.array(list(map(dict_lat.get, obs_ant2)))
            arr_lon2 = np.array(list(map(dict_lon.get, obs_ant2)))
            arr_hgt2 = np.array(list(map(dict_hgt.get, obs_ant2)))

            loc1 = EarthLocation(
                lat=arr_lat1 * au.deg,
                lon=arr_lon1 * au.deg,
                height=arr_hgt1 * au.m
            )

            loc2 = EarthLocation(
                lat=arr_lat2 * au.deg,
                lon=arr_lon2 * au.deg,
                height=arr_hgt2 * au.m
            )

            scd1 = source_coord.transform_to(
                AltAz(obstime=obs_times, location=loc1)
            )

            scd2 = source_coord.transform_to(
                AltAz(obstime=obs_times, location=loc2)
            )

            mask_elevation = (
                (scd1.alt.value >= self.ellim)
                & (scd2.alt.value >= self.ellim)
            )
            ...
            lst1 = atime(obs_times).sidereal_time("apparent", "greenwich")
            lst2 = atime(obs_times).sidereal_time("apparent", "greenwich")
            hangle = (lst1 - ra).to(au.deg)
            ...
            obs_v = (
                - xw * np.sin(dec) * np.cos(hangle)
                + yw * np.sin(dec) * np.sin(hangle)
                + zw * np.cos(dec)
            )

            obs_w = (
                + xw * np.cos(dec) * np.cos(hangle)
                - yw * np.cos(dec) * np.sin(hangle)
                + zw * np.sin(dec)
            )

            mjd = atime(obs_times, format="iso").mjd
            times = (mjd - int(np.min(mjd))) * 24
            tint = np.ones(times.shape[0]) * self.tint
            freq_array = np.full(times.shape[0], frequency / 1e9)
            obs_ant1 = np.array(obs_ant1)[mask_elevation]
            obs_ant2 = np.array(obs_ant2)[mask_elevation]
            obs_ant1_name = np.array(obs_ant1_name)[mask_elevation]
            obs_ant2_name = np.array(obs_ant2_name)[mask_elevation]
            phi1 = np.zeros(times.shape[0])
            phi2 = np.zeros(times.shape[0])

            uvcov = gv.utils.structured_array(
                data=[
                    mjd, times, tint, freq_array, obs_ant1, obs_ant2,
                    obs_ant1_name, obs_ant2_name,  obs_u, obs_v, obs_w,
                    phi1, phi2, scd1.alt.value, scd2.alt.value
                ],
                field=[
                    "mjd", "time", "tint", "frequency", "ant1", "ant2",
                    "ant1_name", "ant2_name", "u", "v", "w",
            ...
            if nf == 0:
                ...
                self.uvcov = rfn.stack_arrays((self.uvcov, uvcov))
...
def get_vlbi(name):
    ...
    arr_nam = np.array(list(map(lambda x: str(x.upper()), vlbiarr)))
    arr_num = np.array([number for number in range(1, len(vlbiarr) + 1)])
    for arr_ in vlbiarr:
        arr = arr_.upper()
        loc = EarthLocation(
            lat=dict_lat[arr],
    ...
    out = gv.utils.structured_array(
        data =[arr_nam, arr_num, arr_xpo, arr_ypo, arr_zpo, arr_lat, arr_lon, arr_hgt],
        field=["name", "number", "x", "y", "z", "lat", "lon", "height"],
        dtype=["U32", "i4", "f8", "f8", "f8", "f8", "f8", "f8"]
    )
