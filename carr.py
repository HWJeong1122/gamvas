...
class create_array:
    def __init__(
        self, array=None, tarr=None, date=None, tstart=None, duration=None,
        scanlen=None, tint=None, gaptime=None,
    ...
    def carr(self):
        ...
        self.tarr = tarr
        self.ant_dict_name2num =\
            dict(
                zip(
                    self.tarr["name"],
                    np.arange(self.tarr.shape[0]) + 1
                )
            )
            ...
            sarray(
                data=\
                    [
                        ...
                        obs_ant_name1, obs_ant_name2,
                        obs_uv_u, obs_uv_v, obs_uv_w,
...
def add_vis(inputdat, select):
    ...
    out =\
        rfn.append_fields(
            out,
            "vis_{0}".format(select.lower()),
            np.zeros(out.shape[0], "c8"), usemask=False
        )
    out =\
        rfn.append_fields(
            out,
            "sigma_{0}".format(select.lower()),
            np.zeros(out.shape[0], "f8"), usemask=False
        )
    return out
